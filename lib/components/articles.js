...
export default class Articles extends Component {
  ...
  show(arg = 'search?page=1&') {
    // if arg present - it could be either url or article DomElement
    const article = arg;
    const url = arg;
      ...
      shortNewsData
        .then((data) => {
          ...
          const content = `
            ...
            `;
          this._openArticle(article, content);
        })
        .catch((err) => {
          this._openArticle(article, err);
        });
    } else {
  ...
  _openArticle(article, content) {
    ...
    if (content) {
      shortArticle.innerHTML = content;
    }
